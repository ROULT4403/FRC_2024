...
public class PIDWrist extends ProfiledPIDSubsystem
{
  // The wrist's motor controller is defined here...
  private final CANSparkMax wrist = new CANSparkMax(3, neoMotorType);
private static final ArmFeedforward feedforward = new ArmFeedforward(0.37485, 7.0038, 0.11421, 1.0256); 
...
private static final TrapezoidProfile.Constraints constraints = new TrapezoidProfile.Constraints(2*Math.PI,2*Math.PI)
...
log -> {
  ...
  {
    super(
      new ProfiledPIDController(62.192,0,0, constraints));
  ...
  setGoal(VER CUAL ES EL GOAL ACA);  }
